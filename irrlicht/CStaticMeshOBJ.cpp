...
IMesh* CStaticMeshOBJ::getMesh(s32 frame, s32 detailLevel)
{
	return &Mesh;
}
	...
	while(line)
	{
		...
		if (wordBuffer[0] == 'f' && wordBuffer[1] == 0x0)
		{
			...
			while(face && face[0])
			{
				...
				{
					meshbuffer->drop();
					os::Warning::print("Face with more than 40 face points found s32 file. Not loading.", file->getFileName());
					delete [] buf;
					return false;
				}
			...
			{
				video::S3DVertex v;
				v.Color = video::Color(255,255,255,255);
		...
		if (wordBuffer[0] == '#' || wordBuffer[0] == 'u')
		{
			// comment
			// find end of line
			s32 i=0;
...
void CStaticMeshOBJ::copyWord(c8* outBuf, s32 bufLenght, c8* word)
{
	if (!word)
		return;
...
void CStaticMeshOBJ::copyWordLineEnd(c8* outBuf, s32 bufLenght, c8* word)
{
	if (!word)
		return;
...
void CStaticMeshOBJ::copyWordSpaceEnd(c8* outBuf, s32 bufLenght, c8* word)
{
	if (!word)
		return;
