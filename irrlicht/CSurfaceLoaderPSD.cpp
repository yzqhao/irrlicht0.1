...
{
	#ifdef _DEBUG
	setDebugName("CSurfaceLoaderPSD");
	#endif
}
...
ISurface* CSurfaceLoaderPSD::loadImage(irr::io::IReadFile* file)
{
	...
	if (header.version != 1)
	{
		os::Warning::print("Unsupported PSD file version", file->getFileName());
		return 0;
	}

	if (header.mode != 3 || header.depth != 8)
	{
		os::Warning::print("Unsupported PSD color mode or depth.\n", file->getFileName());
		return 0;
	}
	...
	if (!file->seek(l, true))
	{
		os::Warning::print("Error seeking file pos to image resources.\n", file->getFileName());
		return 0;
	}
	...
	if (!file->seek(l, true))
	{
		os::Warning::print("Error seeking file pos to layer and mask.\n", file->getFileName());
		return 0;
	}
	...
	if (!file->seek(l, true))
	{
		os::Warning::print("Error seeking file pos to image data section.\n", file->getFileName());
		return 0;
	}
	...
	{
		os::Warning::print("Unsupported psd compression mode.\n", file->getFileName());
		return 0;
	}
	...
	{
		// create surface
		surface = video::createSurface(core::dimension2d<s32>(header.width, header.height));
		CColorConverter::convert32BitTo16BitColorShuffle((c8*)((void*)imageData),
			surface->lock(), header.width, header.height, 0);
	}
...
bool CSurfaceLoaderPSD::readRawImageData(irr::io::IReadFile* file)
{
	...
	for (s32 channel=0; channel<header.channels && channel < 3; ++channel)
	{
		...
		{
			os::Warning::print("Error reading color channel\n", file->getFileName());
			break;
		}
	...
	*/
	...
	for (u32 y=0; y<header.height * header.channels; ++y)
	{
		if (!file->read(&rleCount[y], sizeof(u16)))
		{
			delete [] tmpData;
			delete [] rleCount;
			os::Warning::print("Error reading rle rows\n", file->getFileName());
			return false;
		}
